...
function UpdatePdf() {
    ...
    const customPdfHandle = (e) => {
        const forPdf = UMCData.allCourse.find(course => course._id == formValue.course).parts.find(part => (part._id == formValue.part)).chapters.find(chapter => (chapter._id == formValue.chapter)).classes.find(classItem => (classItem._id == formValue.class)).pdfs.find(pdf => pdf._id == e.target.value)
        setFormValue({ ...formValue, pdf: e.target.value, title: forPdf.title, link: forPdf.link })
    }

    return (
        sectionLoading ? <LoadingScreen /> :
            <form className="px-4 py-8 lg:px-0" method="post" onSubmit={handleSubmit}>
                <h3 className="text-gray-700 text-xl font-medium text-center mb-4">Update PDF</h3>
                ...
                <div className="flex flex-wrap -mx-3 mb-6">
                    <div className="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                        ...
                        <div className="relative">
                            <select className="block appearance-none w-full bg-gray-200 border border-gray-700 text-black py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state" name="pdf" onChange={(e) => customPdfHandle(e)} value={formValue.pdf} required >
                                <option key={Math.random()} value="">Select A PDF</option>
                                {
                                    formValue.class && UMCData.allCourse.find(course => course._id == formValue.course).parts.find(part => (part._id == formValue.part)).chapters.find(chapter => (chapter._id == formValue.chapter)).classes.find(classItem => (classItem._id == formValue.class)).pdfs.map(pdf => (
                                        <option key={Math.random()} value={pdf._id}>{pdf.title}</option>
                                    ))
                ...
                <div className="flex flex-wrap -mx-3 my-2 mt-8">
                    <div className="w-full md:w-full px-3 mb-6 md:mb-0">
                        <input className="flex cursor-pointer items-center justify-center h-12 px-6 w-full bg-blue-600 rounded font-semibold text-sm text-blue-100 hover:bg-blue-700" id="grid-first-name" type="submit" value="Update Pdf" />
                    </div>
                </div>
            </form >
