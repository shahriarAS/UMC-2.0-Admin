...
function ManageOrder({ setDisapprovedOrder }) {
  ...
  const grabOrder = () => {
    axios
      .get(`${process.env.REACT_APP_API_DOMAIN}/order/view`, {
        headers: {
      ...
      .then((response) => {
        ...
        const disapprovedOrderCount = response.data.result.filter(
          (i) => i.order_status != "approved"
        ).length;
        setDisapprovedOrder(disapprovedOrderCount);
      })
      .catch((error) => {
        // console.log(error)
      });
  ...
  return (
    <>
      {openModal ? (
        <OrderDetail setModal={setOpenModal} modalData={modalData} />
      ) : null}
              ...
              <div className="align-middle rounded-tl-lg rounded-tr-lg inline-block w-full py-4 overflow-hidden bg-white shadow-lg px-12">
                <div className="flex justify-between items-center">
                  ...
                  <div>
                    <p>
                      Disapproved Order:{" "}
                      {data.filter((i) => i.order_status != "approved").length}
                    </p>
                  </div>
                </div>
              </div>
