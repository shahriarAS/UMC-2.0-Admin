...
function DeletePart() {
    ...
    const handleSubmit = (e) => {
        ...
        if (formValue.course && formValue.part && deleteConfirm != null && deleteConfirm == "DELETE") {
            setSectionLoading(true)
            axios.delete(`${process.env.REACT_APP_API_DOMAIN}/part/delete/${formValue.part}`, {
                headers: {
    ...
    return (
        sectionLoading ? <LoadingScreen /> :
            <form className="px-4 py-8 lg:px-0" method="post" onSubmit={handleSubmit}>
                <h3 className="text-gray-700 text-xl font-medium text-center mb-4">Delete Part</h3>
                ...
                <div className="flex flex-wrap -mx-3 my-2 mt-8">
                    <div className="w-full md:w-full px-3 mb-6 md:mb-0">
                        <input className="flex cursor-pointer items-center justify-center h-12 px-6 w-full bg-blue-600 rounded font-semibold text-sm text-blue-100 hover:bg-blue-700" id="grid-first-name" type="submit" value="Delete Part" />
                    </div>
                </div>
            </form>
